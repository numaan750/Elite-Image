...
//   const uploadAllToCloudinary = async (images) => {
//   const CLOUD_NAME = "drh7q62eh";
//   const UPLOAD_PRESET = "unsigned_preset";
//   const tag = `share_${Date.now()}`; // unique tag = folder ki tarah kaam karega
  
//   const urls = [];
//   for (let i = 0; i < images.length; i++) {
//     const formData = new FormData();
//     const response = await fetch(images[i].processedImage);
//     const blob = await response.blob();
//     formData.append("file", blob);
//     formData.append("upload_preset", UPLOAD_PRESET);
//     formData.append("tags", tag); // sab images same tag se group hongi
...
//   // Cloudinary media explorer link with tag filter
...
//     setShareLink(singleImageUrl);
//   } else if (processedData.length > 1) {
//     // Multiple images: pehle ZIP banao, phir message show karo
//     setShareLink(""); // blank rakhein jab tak upload na ho
//   }
// }, [formData]);

// const handleCreateShareableAlbum = async () => {
//   setIsPreparingZip(true);
//   toast.loading("Creating shareable album...", { id: "album" });
  
//   try {
//     const CLOUD_NAME = "drh7q62eh";
//     const UPLOAD_PRESET = "unsigned_preset";
//     const tag = `album_${Date.now()}`;
    
//     for (let i = 0; i < processedData.length; i++) {
//       toast.loading(`Uploading image ${i + 1} of ${processedData.length}...`, { id: "album" });
//       const formData = new FormData();
...
//     // Ye link sari images ek saath dikhata hai
//     const albumLink = `https://res.cloudinary.com/${CLOUD_NAME}/image/list/${tag}.json`;
//     setShareLink(albumLink);
//     await navigator.clipboard.writeText(albumLink);
//     toast.success("Album link copied!", { id: "album" });
//   } catch {
//     toast.error("Album creation failed", { id: "album" });
//   } finally {
